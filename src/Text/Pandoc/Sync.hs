...
----
---- Okay, new dep-tracking inspired style:
----
---- 1. Parse all real copies into canonical rep
---- 2. Merge all into canonical rep
---- 3. All real copies updated from canonical rep
----
---- Maybe if we use dep-tracking style, everything will take care of itself.
---- The only thing that is weird is the awkward cyclic dependency.
----
---- But maybe it is not so cylc.  "New" doc.md is dependent on "old" one,
---- and that's ok?
...
--    -> (FilePath -> m CFile)
--    -> m CFile
--buildCanonical fetch = undefined
